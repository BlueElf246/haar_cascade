...
class ViolaJones:
    ...
    def train(self, training, pos_num, neg_num):
        ...
        print("apply features to training examples")
        X,y= self.apply_feature(features, training_data)
        print("select best features")
        indices= SelectPercentile(f_classif, percentile=10).fit(X.T,y).get_support(indices=True)
        X=X[indices]
        features=features[indices]
        print(f"selected {len(X)} potential features")
        for t in range(self.T):
            ...
            print(f"choose classifer: {str(clf)} with accuracy {len(accuracy)- sum(accuracy)} and alpha {alpha}")
    ...
    def apply_feature(self, features, training_data):
    ...
    def train_weak(self, X, y, features, weights):
        ...
        for index, feature in enumerate(X):
            if len(classifiers) % 1000==0 and len(classifiers)!=0:
                print(f'train {len(classifiers)} out of {total_features}')
